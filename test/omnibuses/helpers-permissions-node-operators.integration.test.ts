...
import { getOptionalEnvVar } from "../../src/common/env";
...
describe("permissions and node operator helpers (mainnet fork integration)", function () {
  ...
  before(async function () {
    const rpcUrl = getOptionalEnvVar("ETH_MAINNET_RPC_URL");
    if (rpcUrl === undefined) {
      this.skip();
    }
    connection = await hre.network.connect({
      network: "default",
      override: { chainId: 31337, forking: { enabled: true, url: rpcUrl.split(",")[0], blockNumber: FORK_BLOCK } },
    });
