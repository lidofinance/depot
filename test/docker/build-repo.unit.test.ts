...
function cachedImage(repo: string, revision: string): Docker.ImageInfo {
  return {
    Id: "cached-image",
    ParentId: "",
    RepoTags: [`depot/${repo}:${SHA.slice(0, 7)}`],
    Created: 1,
    Size: 0,
    VirtualSize: 0,
    SharedSize: 0,
...
describe("repository image builds", () => {
  ...
  beforeEach(() => {
    process.env = { ...originalEnv, GITHUB_ORG: "lidofinance", GIT_SHA_STAKING_MODULES: "", GIT_SHA_STONKS: "" };
  ...
  for (const repo of ["staking-modules", "stonks"] as const) {
    it(`pins ${repo} to the full SHA resolved from its branch`, async () => {
      ...
      assert.equal(tag, `depot/${repo}:${SHA.slice(0, 7)}`);
