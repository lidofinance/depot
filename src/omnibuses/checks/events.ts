import { BaseContract, EventFragment, EventLog, Interface, Log, TransactionReceipt } from 'ethers'
...
const checkOmnibusEvents = ({ contracts }: CheckContext, actions: OmnibusItem[], receipt: TransactionReceipt) => {
...
function isArgsMatches(log: Log, fragment: EventFragment, eventArgs: unknown[]): boolean {
  ...
  for (let i = 0; i < eventArgs.length; ++i) {
    ...
    if (!bytes.isEqual(logArg as string, eventArgValue as string)) return false
  }
  return true
}
