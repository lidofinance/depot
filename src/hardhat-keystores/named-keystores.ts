...
export function create(storage: NamedKeystoresStorage): NamedKeystores {
  function all(): Promise<NamedKeystore[]> {
    return storage.all()
  }

  function get(name: string): Promise<NamedKeystore | null> {
    return storage.get(name)
  }

  // getOrSelect will either return the keystore with the given name or prompt the user to select one
