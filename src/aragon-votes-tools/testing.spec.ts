...
type RpcMockRequest = { payload: JsonRpcPayload; response: JsonRpcResult; used?: number }

class MockProvider extends JsonRpcProvider {
  snapshot: Record<number, RpcMockRequest> = {}
  private rpcUrl: string = ''
  private blockNumber: string = '' // use for mocking blockNumber requests in random order
  setSnapshot(snapshot: Record<string, RpcMockRequest>) {
  ...
  sendOne(payload: JsonRpcPayload): Promise<JsonRpcResult> {
...
describe('testing-tools', () => {
  const ignoreMethod = ['eth_blockNumber', 'eth_chainId']
  it('adopt', async () => {
    const provider = new MockProvider()
    provider.setSnapshot(mocks.adopt as Record<string, RpcMockRequest>)
    ...
    const unUsedMockIds = Object.values(provider.snapshot)
      .filter((item) => !item.used && !ignoreMethod.includes(item.payload.method))
    ...
    const multiUsedMockIds = Object.values(provider.snapshot)
      .filter((item) => item?.used && item.used > 1 && !ignoreMethod.includes(item.payload.method))
      .map((item) => item.payload.id)
