...
function sendMine(node: LocalNodeInfo, provider: RpcProvider, blocks: number) {
  switch (node.name) {
    case 'anvil':
...
function sendSetCode(node: LocalNodeInfo, provider: RpcProvider, address: Address, code: string): Promise<unknown> {
...
function sendLock(node: LocalNodeInfo, provider: RpcProvider, address: Address): Promise<unknown> {
  switch (node.name) {
    case 'anvil':
...
export function cheats(provider: RpcProvider): ProviderCheats {
  ...
  async function snapshot(): Promise<SnapshotRestorer> {
    let snapshotId: string = await provider.send('evm_snapshot', [])
