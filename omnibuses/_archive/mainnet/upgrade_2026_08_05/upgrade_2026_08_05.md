...
- Fork block for the test: 25724758 (the block before the execution). The test also checks the contract against the on-chain script with `isValidVoteScript(204)`:
  ...
  Historical reproduction command, requiring the P8 `--fork-block` support (absent from this slice’s P6b base):

  ```bash
  npm run omnibus:test -- upgrade_2026_08_05 --fork-block 25724758
  ```

- On this slice, run `npm run omnibus:test -- upgrade_2026_08_05` only with an already-running local fork at block 25724758 and the target-network RPC configuration. Without that prepared node, the fallback forks latest and does not reproduce the recorded before-state. The script comparison below is retained historical evidence, not a new fork run.
